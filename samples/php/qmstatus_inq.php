<?php
	...
	if ( ($response = curl_exec($curl)) === false )	{
		$err = curl_error($curl);
		echo "An HTTP error occurred while inquiring queuemanager status: $err\n";
	}
	else {
		...
		if ( isset($json->error) ) {
			...
			if ( isset($json->status) ) {
				echo "{$json->status->QMgrName->value} started on {$json->status->StartDate->value} {$json->status->StartTime->value}\n";
			}
			else
			{
