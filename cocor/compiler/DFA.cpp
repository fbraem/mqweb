...
void DFA::Step(State *from, Node *p, BitArray *stepped) {
	...
	if (p->typ == Node::clas || p->typ == Node::chr) {
		...
		if (p->state != from) {
			BitArray *newStepped = new BitArray(tab->nodes.size());
...
void DFA::FindTrans (Node *p, bool start, BitArray *marked) {
	...
	marked->Set(p->n, true);
	if (start) {
		BitArray *stepped = new BitArray(tab->nodes.size());
...
void DFA::ConvertToStates(Node *p, Symbol *sym) {
  ...
  if (tab->DelGraph(curGraph)) {
	...
	FindTrans(curGraph, true, new BitArray(tab->nodes.size()));
	if (p->typ == Node::iter) {
		BitArray *stepped = new BitArray(tab->nodes.size());
...
void DFA::MatchLiteral(const std::string& s, Symbol *sym) {
  std::string subS(s, 1, s.length()-2);
  std::string s1 = tab->Unescape(subS);
  int i, len = s1.length();
	State *state = firstState;
	Action *a = NULL;
	for (i = 0; i < len; i++) { // try to match s against existing DFA
		a = FindAction(state, s1[i]);
		if (a == NULL) break;
	...
	for (; i < len; i++) { // make new DFA for s[i..len-1]
		State *to = NewState();
		NewTransition(state, to, Node::chr, s1[i], Node::normalTrans);
		state = to;
	}
    ...
    std::stringstream ss;
    ss << "tokens " << sym->name << " and " << matchedSym->name << "  cannot be distinguished";
		parser->SemErr(ss.str());
...
void DFA::PrintStates() {
	...
	for (State *state = firstState; state != NULL; state = state->next) {
      ...
      std::string paddedName = tab->Name(state->endOf->name);
      fprintf(trace, "E(%12s)", paddedName.c_str());
		}
      ...
      if (action->typ == Node::clas) fprintf(trace, "%s", tab->classes[action->sym]->name.c_str());
      else fprintf(trace, "%3s", Ch(action->sym).c_str());
			for (Target *targ = action->target; targ != NULL; targ = targ->next) {
...
void DFA::GetTargetStates(Action *a, BitArray* &targets, Symbol* &endOf, bool &ctx) {
	...
	for (Target *t = a->target; t != NULL; t = t->next) {
        ...
        std::cout << "Tokens " << endOf->name << " and " << t->state->endOf->name << " cannot be distinguished" << std::endl;
				errors->count++;
			}
		}
		if (t->state->ctx) {
...
void DFA::GenComBody(Comment *com) {
  ...
  fprintf(gen, "\t\t\tif (%s) ", res.c_str());
	...
	fprintf(gen, "\t\t\t} else if (ch == (COCO_WCHAR_MAX + 1)) return false;\n");
...
void DFA::GenComment(Comment *com, int i) {
  ...
  if (com->start.length() == 1) {
    ...
    fprintf(gen, "\tif (%s) ", res.c_str());
		...
		fprintf(gen, "\t} else {\n");
		fprintf(gen, "\t\buffer.seekg(pos0, std::ios::beg); NextCh(); line = line0; col = col0; charPos = charPos0;\n");
		fprintf(gen, "\t}\n");
...
std::string DFA::SymName(Symbol *sym) { // real name value is stored in Tab.literals
	if (    ('a' <= sym->name[0] && sym->name[0] <= 'z') 
       || ('A' <= sym->name[0] && sym->name[0] <= 'Z')) //Char::IsLetter(sym->name[0])
  {
    for(SymbolHashMap::Iterator it = tab->literals.begin(); it != tab->literals.end(); it++)
    {
      if ( it->second == sym )
      {
        return it->first;
	...
	for (int i = 0; i < 2; ++i) {
        ...
        std::string name = SymName(sym);
          ...
          std::string oldName = name;
				...
				fprintf(gen, "\tkeywords.insert(std::pair<std::string, int>(");
...
int DFA::GenNamespaceOpen(const std::string& nsName) {
  if (nsName.empty() ) {
		return 0;
	}
...
void DFA::WriteState(State *state) {
	...
	for (Action *action = state->firstAction; action != NULL; action = action->next) {
      ...
      std::string res = ChCond(action->sym);
	...
	if (ctxEnd) { // final context state: cut appendix
		...
		fprintf(gen, "\t\t\t\tbuffer.seekg(t->pos, std::ios::beg); NextCh(); line = t->line; col = t->col;\n");
	...
	if (endOf == NULL) {
		...
		if (endOf->tokenKind == Symbol::classLitToken) {
			if (ignoreCase) {
        fprintf(gen, "std::string literal = tval; std::transform(literal.begin(), literal.end(), literal.begin(), ::tolower); t->kind = getKeywordIdentifier(literal, t->kind); break;}\n");
			} else {
        fprintf(gen, "std::string literal = tval; t->kind = getKeywordIdentifier(literal, t->kind); break;}\n");
			}
		} else {
			fprintf(gen, "break;}\n");
		}
...
void DFA::WriteStartTab() {
	...
	for (Action *action = firstState->firstAction; action != NULL; action = action->next) {
		...
		if (action->typ == Node::chr) {
			fprintf(gen, "\tstart.insert(std::make_pair<int, int>(%d, %d));\n", action->sym, targetState);
		} else {
			...
			for (CharSet::Range *r = s->head; r != NULL; r = r->next) {
				...
				fprintf(gen, "\tfor (i = %d; i <= %d; ++i) start.insert(std::make_pair<int, int>(i, %d));\n", r->from, r->to, targetState);
			}
		}
	}
	fprintf(gen, "\tstart.insert(std::make_pair<int, int>(-1, -1));\n");
}
