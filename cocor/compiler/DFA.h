...
public:
	...
	//---------- Output primitives
  std::string Ch(int ch);
  std::string ChCond(int ch);
  ...
  void MatchLiteral(const std::string& s, Symbol *sym);
  ...
  std::string SymName(Symbol *sym); // real name value is stored in Tab.literals
	void GenLiterals ();
  int GenNamespaceOpen(const std::string& nsName);
	void GenNamespaceClose(int nrOfNs);
	void WriteState(State *state);
	void WriteStartTab();
  void OpenGen(const std::string& genName, bool backUp); /* pdt */
	void WriteScanner();
	DFA(Parser *parser);
};
